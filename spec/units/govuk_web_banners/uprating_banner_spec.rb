RSpec.describe GovukWebBanners::UpratingBanner do
  ...
  describe ".for_path" do
    context "with banners" do
      ...
      it "returns banner that includes the path" do
        expect(described_class.for_path("/calculate-married-couples-allowance")).to be_instance_of(described_class)
      end

      it "returns nil for a path without a banner" do
        expect(described_class.for_path("/married-allowance")).to be_nil
      end
    end

    context "with timed banners" do
      let(:replacement_file) do
        YAML.load_file(Rails.root.join(fixtures_dir, "timed_uprating_banners.yml"))
      end
